...
----------------------------------------------------------------------
notes:
        Not sure if faster to use a 2d matrix or single array with arithmetic
...
if __name__ == '__main__':
    ...
    width = 20
    height = 20
    for line in getFileLines("input/i11.txt"):
    ...
    for row in range(0, 17, 1):
        for col in range(0, 20, 1):
            block = getColumn(data, 4, row, col)
            prod = arrayProduct(block)
            if prod > blockMax:
                blockMax = prod
